...
export const mesh: VisualMode = {
  ...
  draw(f) {
    ...
    const cols = Math.floor(20 + p.dens * 28);
    const rows = Math.floor(12 + p.dens * 12);
    const h0 = f.hue();
    const g = p.gain;
    ...
    const px = (i: number, j: number): [number, number] => {
      ...
      const y = H * 0.2 + fr * fr * H * 0.72 - v * H * 0.12 * (0.3 + fr) - wob;
      return [x, y];
    };
    if (p.vari === 2) {
      /* 交点を光点で打つ */
      for (let j = 0; j < rows; j++) {
        ...
        for (let i = 0; i < cols; i++) {
          ...
          cx.beginPath();
          cx.arc(x, y, (0.8 + fr * 1.8 + v * 2.5 + a.beatEnv * p.punch) * DPR, 0, Math.PI * 2);
          cx.fill();
        }
      }
    } else {
